...
def run_optimization(pred_values, fund_close, risk_preference="high", top_n=5, threshold=0.005):
    """ 
    基于预测结果运行投资组合优化
    ...
    - fund_close: 基金收盘价数据框
    - risk_preference: 风险偏好，'high'或'low'
    - top_n: 选择预测收益率最高的前N个行业进行投资
    ...
    # 如果没有预测收益率大于0的行业，则选择预测收益率最高的前N个行业
    if len(positive_industries) < top_n:
        print(f"预测收益率大于{threshold}的行业不足{top_n}个，将选择预测收益率最高的前{top_n}个行业")
        positive_industries = pred_values.sort_values(ascending=False).head(top_n)
    ...
    # 配置优化参数
    opt_params = {
        "max_weight": config.optimization_params["weight_limit"][risk_preference],
        "min_weight": config.optimization_params["min_weight"][risk_preference],  
    }
...
def generate_historical_weights(predictions_dict, fund_close, start_date=None, end_date=None, 
                               risk_preference="high", top_n=5, threshold=0.005, freq="W-FRI"):
    """
    生成历史上每个时间点的最优权重
    ...
    if historical_weights:
        ...
        # 保存权重结果
        output_weights_file = f"./output/historical_weights_{risk_preference}_top{top_n}.csv"
        ...
        if historical_predictions:
            ...
            output_predictions_file = f"./output/historical_predictions_{risk_preference}_top{top_n}.csv"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="基于GRU预测结果生成最优权重")
    parser.add_argument("--start", type=str, help="开始日期 (YYYY-MM-DD 格式)")
    parser.add_argument("--end", type=str, help="结束日期 (YYYY-MM-DD 格式)")
    parser.add_argument("--top_n", type=int, default=5, help="选择预测收益率最高的前N个行业")
    parser.add_argument("--risk", type=str, default="high", choices=["high", "low"], help="风险偏好")
    parser.add_argument("--freq", type=str, default="W-FRI", help="生成权重的频率")
    parser.add_argument("--threshold", type=float, default=0.005, help="预测收益率阈值")
    ...
    try:
        # 尝试从models目录加载预测结果
        predictions = torch.load(f"./models/all_industry_predictions_{freq}.pt")
        ...
        try:
            ...
            # 运行优化，只选择前N个行业
            run_optimization(latest_predictions, fund_close, risk_preference=args.risk, top_n=args.top_n)
