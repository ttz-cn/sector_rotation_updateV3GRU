...
def run_optimization(pred_values, fund_close, risk_preference="high", top_n=5, threshold=0.015):
    """ 
    基于预测结果运行投资组合优化
    ...
    if len(positive_industries) > top_n:
        print(f"预测收益率大于{threshold}的行业超过{top_n}个，将只选择预测收益率最高的前{top_n}个行业")
        positive_industries = positive_industries.sort_values(ascending=False).head(top_n)
    # 如果大于threshold的行业不足3个，则选择3个 
    elif len(positive_industries) < 3 :
        print(f"预测收益率大于{threshold}的行业不足3个，将选择3个")
        positive_industries = pred_values.sort_values(ascending=False).head(3)
...
def generate_historical_weights(predictions_dict, fund_close, start_date=None, end_date=None, 
                               risk_preference="high", top_n=5, threshold=0.005, freq='B'):
    """
    生成历史上每个时间点的最优权重
    ...
    if historical_weights:
        ...
        # 保存权重结果
        output_weights_file = f"./output/historical_weights_{risk_preference}_top{top_n}_{threshold}.csv"
        ...
        if historical_predictions:
            ...
            output_predictions_file = f"./output/historical_predictions_{risk_preference}_top{top_n}_{threshold}.csv"
